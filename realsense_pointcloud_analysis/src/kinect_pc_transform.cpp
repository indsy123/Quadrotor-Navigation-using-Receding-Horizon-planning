...
void depthcallback_realsense(const sensor_msgs::PointCloud2ConstPtr& cloud)
{	
	// define transform as StampedTransform
	tf::StampedTransform transform;
	printf ("Cloud: frameid = %s\n", cloud->header.frame_id);
	...
	listener_center_bumper_ptr->lookupTransform("camera_depth_frame", "camera_depth_optical_frame", ros::Time(0), transform);
	
	//transform pointclouds
	pcl_ros::transformPointCloud(cloud_in, cloud_trans, transform);
	cloud_trans.header.frame_id="camera_depth_frame";  // specify the frame id
...
{	
	...
	sensor_msgs::PointCloud2 transformed_cloud;
	pcl::toROSMsg(cloud_trans, transformed_cloud);
	pub2.publish(transformed_cloud);
}
...
int main (int argc, char** argv)
{
  ...
  pub2 = nh.advertise<sensor_msgs::PointCloud2>("/final_c2", 1, true);

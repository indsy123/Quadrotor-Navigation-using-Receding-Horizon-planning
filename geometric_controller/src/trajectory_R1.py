...
class trajectory(object): 
	...
	self.w = 2*np.pi/self.T
        try:
	    rospy.Subscriber('/'+self.uav+'/modifiedodom', Odometry, self.callback, queue_size = 100, tcp_nodelay = True)
	...
	if tt<=self.T: 
            msg.pdes.x = 0.0#0.9 * np.cos(np.pi*(tt)/3)
	    msg.pdes.y = 0#0.9 * np.sin(np.pi*(tt)/3)
	    msg.pdes.z = 0.75

            msg.vdes.x = 0#-0.9 * (np.pi/3) * np.sin(np.pi*(tt)/3)
	    msg.vdes.y = 0#0.9 * (np.pi/3) * np.cos(np.pi*(tt)/3)
	    msg.vdes.z = 0

            msg.ades.x = 0#-0.9 * (np.pi/3)**2 * np.cos(np.pi*(tt)/3)
	    msg.ades.y = 0#-0.9 * (np.pi/3)**2 * np.sin(np.pi*(tt)/3)
	    msg.ades.z = 0.0

            msg.ddes.x = 1
            msg.ddes.y = 0
            msg.ddes.z = 0
	#elif tt > self.T and tt <=2*self.T: 
        #    msg.pdes.x = 0.5
	#    msg.pdes.y = 0
	#    msg.pdes.z = 0.75

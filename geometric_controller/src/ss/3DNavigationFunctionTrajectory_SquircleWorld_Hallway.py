...
class NavigationFunction_Trajectory3D(object):
    def __init__(self, name_of_uav, time_instance): 
        ...
        self.WS = np.array([0.0, 0.0, 0.6, 0.5, 0.995, 0, 0.1, 1.0, 0.8333]) # workspace
        self.O1 = np.array([-3.0, -0.2, 0.35, 0.24, 0.995, 0, 1.0, 1.0, 0.6857]) #obstacle 1, leave radius 1 ft
        self.O2 = np.array([-1.0, 0.2, 0.35, 0.24, 0.995, 0, 1.0, 1.0, 0.6857]) #obstacle 2, leave radius 1 ft
        self.O3 = np.array([1.0, -0.2, 0.35, 0.24, 0.995, 0, 1.0, 1.0, 0.6857]) #obstacle 3, leave radius 1 ft        
        self.O4 = np.array([3.0, 0.2, 0.35, 0.24, 0.995, 0, 1.0, 1.0, 0.6857]) #obstacle 4, leave radius 1 ft  
        
        
        self.n = 5; self.frame = 1
        #self.goal = np.array([0.75, 0.75, 0.1]); self.k = 4.5; self.lambda_sq = 1.0; self.lambda_sp = 30.0
        self.goal = np.array([0, -0.2, 0.3]); self.k = 15; self.lambda_sq = 1e10; self.lambda_sp = 1e10
        ...
        self.rho2 = self.O2[3] / max(self.O2[6], self.O2[7], self.O2[8]) - 0.05
        self.rho3 = self.O3[3] / max(self.O3[6], self.O3[7], self.O3[8]) - 0.05
        self.rho4 = self.O3[3] / max(self.O4[6], self.O4[7], self.O4[8]) - 0.05     
        #center of star and spheres used in transformations
        self.p0 = self.q0 = np.array([self.WS[0], self.WS[1], self.WS[2]+0.005])
        self.p1 = self.q1 = np.array([self.O1[0], self.O1[1], self.O1[2]]) 
        self.p2 = self.q2 = np.array([self.O2[0], self.O2[1], self.O2[2]]) 
        self.p3 = self.q3 = np.array([self.O3[0], self.O3[1], self.O3[2]]) 
        self.p4 = self.q4 = np.array([self.O4[0], self.O4[1], self.O4[2]]) 
        ...
        self.time = time.time(); self.scale_factor = 5 # scaling between actual WS and navigation function WS
        self.dX = 0.001/self.scale_factor
        self.VelocityMagnitude = 0.5 # magnitude of desired velocity in m/s
        ...
        self.proximity = 0.005; self.dummy_counter = 1.0
    ...
    def calculate_analytic_switch(self, c_, beta0, beta1, beta2, beta3, beta4): 
        ...
        return _s0, _s1, _s2, _s3, _s4, _sd#, beta_bar_0, beta_bar_1
    ...
    def calculate_position_trajectory(self, X_, odomm): 
        ...
        #direction_vector = (self.goal-X_)/nrm(self.goal-X_)
        position = self.scale_factor*X_*0.3048
    ...
    def callback(self, odom): 
        ...
        X = Xprime/self.scale_factor/0.3048 # convert metres to feet
        #print X
        ...
        T0, T1, T2, T3, T4 = self.translated_scaling_map(c, beta_ws, beta_o1, beta_o2, beta_o3, beta_o4)

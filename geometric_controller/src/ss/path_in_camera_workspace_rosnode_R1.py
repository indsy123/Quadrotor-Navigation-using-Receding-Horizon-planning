...
class trajecotory_in_camera_workspace(object): 
    def __init__(self, name, number, ip, io, res, radius, sr, fov_x, fov_y, h, vel, goal): 
        ...
        self.pcl_pub = rospy.Publisher('/firefly'+self.no +'/vi_sensor/camera_depth/depth/cloud_for_octomap', PointCloud2, queue_size = 100, tcp_nodelay = True)
        self.uav_traj_pub = rospy.Publisher('/firefly'+self.no +'/polynomial_trajectory', MultiDOFJointTrajectory, queue_size = 100, tcp_nodelay = True)
        self.target_odom = rospy.Publisher('/firefly'+self.no +'/target/odometry', Odometry, queue_size = 100, tcp_nodelay = True)
        ...
        try:
            ...
            rospy.Subscriber('/firefly'+self.no + '/vi_sensor/camera_depth/depth/voxel_points', PointCloud2, self.planning_callback, queue_size = 1, tcp_nodelay = True)
            rospy.Subscriber('/firefly'+self.no +'/odometry_sensor1/odometry', Odometry, self.odometry_callback, queue_size = 1, tcp_nodelay = True)
        except:

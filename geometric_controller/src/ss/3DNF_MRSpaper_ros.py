...
class squircleworld_NF(object):
    def __init__(self, name_of_uav, number, velocity, initial_hovering_height, freq): 
        ...
        self.k = 40.0#13
        self.lambda_sq = 1e8#5e7#1e6# 1e4
        self.lambda_sp = 1e8#5e7 # 1e4
        """
        ...
        #self.pub = rospy.Publisher('/reached_goal', Point, queue_size = 1, tcp_nodelay = True)
    ...
    def generate_velocity_trajectory(self, phi_, X_, odom): 
        ...
        smoothing_factor = 1/(1+2*np.exp(-2*(t-1)))*scipy.special.erf(6.0*(dist_to_goal-self.proximity))     

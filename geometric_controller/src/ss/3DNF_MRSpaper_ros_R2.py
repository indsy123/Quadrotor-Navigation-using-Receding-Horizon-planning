...
from geometric_controller.msg import velocities_from_navigation_function, target_positions_5quad
...
class squircleworld_NF(object):
    def __init__(self, name_of_uav, number, velocity, initial_hovering_height, freq): 
        ...
        self.k = 19#40
        self.lambda_sq = 2e5#4e5
        self.lambda_sp = 1e12#1e9
        """
        self.no_of_obstacles = 19 # this number includes workspace boundary
        self.O= [\
                 [0.0, 0.0, 15.0, 15.0, 0.9999, 0.0, 1.0, 1.0, 1.0], \
                 [-6.0, -8.0, 4.05, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.2], \
                 [-3.0, -8.0, 3.05, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.266], \
                 [3.0, -8.0, 2.05, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.4], \
                 [6.0, -8.0, 1.05, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.8], \
                 [-4.5, -4.0, 1.05, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.8], \
                 [0.0, -4.0, 2.05, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.4], \
                 [4.5, -4.0, 3.05, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.266], \
                 [-6.0, 0.0, 1.55, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.533], \
                 [-3.0, 0.0, 1.05, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.8], \
                 [3.0, 0.0, 1.05, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.8], \
                 [6.0, 0.0, 2.55, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.32], \
                 [-4.5, 4.0, 2.55, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.32], \
                 [0.0, 4.0, 1.55, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.533], \
                 [4.5, 4.0, 1.05, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.8], \
                 [-6.0, 8.0, 3.05, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.266], \
                 [-3.0, 8.0, 4.05, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.2], \
                 [3.0, 8.0, 2.05, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.4], \
                 [6.0, 8.0, 1.05, 0.8, 0.9999, 0.0, 1.0, 1.0, 0.8] \
                 ]
        self.k = 29#13
        self.lambda_sq = 2e5#1e6# 1e4
        self.lambda_sp = 1e12 # 1e4
        """
        ...
        try:
            rospy.Subscriber('/target_positions_for_quads', target_positions_5quad, self.callback1, queue_size = 1, tcp_nodelay = True)
    ...
    def callback1(self, tp): 
        ...
        if self.no == '1': 
            ...
            self.goal = np.array([tp.robot4.x, tp.robot4.y, tp.robot4.z])/self.scale_factor
        elif self.no == '5':
            self.goal = np.array([tp.robot5.x, tp.robot5.y, tp.robot5.z])/self.scale_factor

...
class velocity_controller(object): 
    def __init__(self, name_of_uav, number, freq, parameters): 
        ...
        #rospy.Subscriber('/polynomial_trajectory', PolynomialTrajectory, self.traj_callback, queue_size  = 1, tcp_nodelay = True)
    ...
    def traj_callback(self, traj): 
        ...
        for i in range(len(traj.points)): 
            ...
            f4.write("%s, %s, %s, %s, %s, %s, %s, %s, %s, %s\n" % (tt+self.initial_odom_time, posx, posy, posz, velx, vely, velz, accx, accy, accz))
        ...
        """
        self.dpos = ar([[traj.pdes.x], [traj.pdes.y], [traj.pdes.z]])
        self.dvel = ar([[traj.vdes.x], [traj.vdes.y], [traj.vdes.z]])
        self.dacc = ar([[traj.ades.x], [traj.ades.y], [traj.ades.z]])
        self.ddir = ar([[traj.ddes.x], [traj.ddes.y], [traj.ddes.z]])
        self.controller = traj.controller
        f4 = open('trajectory_generated_subscribed.txt','a')
        f4.write("%s, %s, %s, %s, %s, %s, %s, %s, %s\n" % (traj.pdes.x, traj.pdes.y, traj.pdes.z, traj.vdes.x, traj.vdes.y, traj.vdes.z, traj.ades.x, traj.ades.y, traj.ades.z))
        """
    ...
    def odom_callback(self, odom): 
        ...
        #Xd = self.dpos; Vd = self.dvel; ad = self.dacc; b1d = self.ddir

...
class trajecotory_in_camera_workspace(object): 
    def __init__(self): 
        ...
        self.radius_of_obstacle = 0.415 # just for testing I am using 0.1315 instead of 0.315
        ...
        self.pcl_pub = rospy.Publisher('/firefly/vi_sensor/camera_depth/depth/cloud_for_octomap', PointCloud2, queue_size = 100, tcp_nodelay = True)
        self.uav_traj_pub = rospy.Publisher('/polynomial_trajectory', MultiDOFJointTrajectory, queue_size = 100, tcp_nodelay = True)
        #self.custom_traj_pub = rospy.Publisher('/polynomial_trajectory_custom_msg', PolynomialTrajectory, queue_size = 200, tcp_nodelay = True)
        
        try:
            #rospy.Subscriber('/firefly/vi_sensor/camera_depth/depth/points', PointCloud2, self.planning_callback, queue_size = 2, tcp_nodelay = True)
            rospy.Subscriber('/firefly/vi_sensor/camera_depth/depth/voxel_points', PointCloud2, self.planning_callback, queue_size = 1, tcp_nodelay = True)
            rospy.Subscriber('/firefly/odometry_sensor1/odometry', Odometry, self.odometry_callback, queue_size = 1, tcp_nodelay = True)
        except:
    ...
    def generate_regular_pyramid_grid(self): 
        ...
        # add the initial point to the grid. this is the (0,0,0) in the first 
        # time step and the end of execution point in subsequent time steps
        # this is not the quadrotor current position, will have to see what to 
        # do for that.
        
        #points_in_pyramid = np.concatenate((points_in_pyramid, self.p_eoe), 0)
        for zz in z: 
    ...
    def trajectory_in_camera_fov(self, data):
        ...
        #self.end_point = self.end_point + np.array([-self.target_velocity*0.1,0, 0])        
        start = time.time()
        ...
        try: 
            ...
            T, p1, p2, p3 = Minimum_snap_trajetory(path, waypoint_specified, waypoint_bc, self.v_eoe, self.a_eoe, self.j_eoe).construct_polynomial_trajectory()
            ...
            print 'total time taken in the loop is:', time.time()-start
        except: 
            ...
            vector = np.array([self.end_point[0]-self.Pglobal[0], self.end_point[1]-self.Pglobal[1], self.end_point[2]-self.Pglobal[2]])
            direction = vector/np.linalg.norm(vector)
            transforms = Transform(translation = Point(self.Pglobal[0], self.Pglobal[1], self.Pglobal[2]), rotation = Quaternion(0,0,0,1))
            velocities = Twist(linear = Point(0, 0, 0), angular = Point(0,0,0))
            accelerations = Twist(linear = Point(0, 0, 0), angular = Point(direction[0],direction[1],direction[2]))
    ...
    def hover_the_quad(self): 
        ...
        transforms = Transform(translation = Point(-5.0, 4, 2), rotation = Quaternion(0,0,0,1))
        velocities = Twist(linear = Point(0, 0, 0), angular = Point(0,0,0))

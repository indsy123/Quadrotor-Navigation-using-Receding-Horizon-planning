...
class NavigationFunction_Trajectory3D(object):
    def __init__(self, name_of_uav, time_instance): 
        ...
        try:
            odom = rospy.Subscriber('/'+self.uav+'/vi_sensor/ground_truth/odometry', Odometry, self.callback, tcp_nodelay = False)
            rospy.spin()
        except:
    ...
    def calculate_velocity_trajectory(self, phi_, X_): 
        ...
        if nrm(X_-self.goal) > self.proximity and self.phase_one == False:
            ...
            smoothing_factor = 1/(1+3*np.exp(-3*(t-2)))*scipy.special.erf(20*(dist_to_goal-self.proximity))
    ...
    def callback(self, odom): 
        ...
        #a = time.time()
        Xprime = np.array([odom.pose.pose.position.x-0.1, odom.pose.pose.position.y, odom.pose.pose.position.z+0.03])

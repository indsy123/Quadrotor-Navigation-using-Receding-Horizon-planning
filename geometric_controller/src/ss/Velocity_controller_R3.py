...
class velocity_controller(object): 
    ...
    def callback(self, odom, odom2, accel, traj): 
        ...
        Omega1 = ar([[odom.twist.twist.angular.x], [odom.twist.twist.angular.y], [odom.twist.twist.angular.z]])
        ...
        intermediate = np.dot(-vi_pos_off_hat.T, vi_rot_off)
        V1 = np.dot(vi_rot_off.T,_V) + np.dot(intermediate, Omega1)  
        
        Omega = np.dot(vi_rot_off.T, Omega1)
        ...
        VV = np.dot(RR, VV)
        V1 = np.dot(R, V1)
        #Omega = ar([[odom2.twist.twist.angular.x], [odom2.twist.twist.angular.y], [odom2.twist.twist.angular.z]])
        #Omega_hat = ar([[0,-Omega[2][0], Omega[1][0]], [Omega[2][0], 0, -Omega[0][0]], [-Omega[1][0], Omega[0][0], 0]])
        #acc = np.dot(R, acc)
        ...
        f.write("%s, %s, %s, %s, %s, %s\n" % (Omega1[0][0], Omega1[1][0], Omega1[2][0],Omega[0][0], Omega[1][0], Omega[2][0]))        
        ...
        f4.write("%s, %s, %s, %s, %s, %s, %s, %s, %s\n" % (_V[0][0], _V[1][0], _V[2][0], V1[0][0], V1[1][0], V1[2][0], VV[0][0], VV[1][0], VV[2][0]))
        ...
        Msg.Moment_y = self.M[1][0] #- 0.14 * np.cos(theta) # this constant torque is added to offset the unbalanced weight of the firefly

...
class squircleworld_NF(object):
    def __init__(self, name_of_uav): 
        ...
        """
        self.no_of_obstacles = 19 # this number includes workspace boundary
               
        self.O= [\
                 [0.0, 0.0, 12.0, 12.0, 0.9999, 0.0, 1.0, 1.0, 1.0], \
                 [-6.0, -8.0, 4.0, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.15], \
                 [-3.0, -8.0, 3.0, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.2], \
                 [3.0, -8.0, 2.0, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.3], \
                 [6.0, -8.0, 1.0, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.6], \
                 [-4.5, -4.0, 1.0, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.6], \
                 [0.0, -4.0, 2.0, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.3], \
                 [4.5, -4.0, 3.0, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.2], \
                 [-6.0, 0.0, 1.5, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.4], \
                 [-3.0, 0.0, 1.0, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.6], \
                 [3.0, 0.0, 1.0, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.6], \
                 [6.0, 0.0, 2.5, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.24], \
                 [-4.5, 4.0, 2.5, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.24], \
                 [0.0, 4.0, 1.5, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.4], \
                 [4.5, 4.0, 1.0, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.6], \
                 [-6.0, 8.0, 3.0, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.2], \
                 [-3.0, 8.0, 4.0, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.15], \
                 [3.0, 8.0, 2.0, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.3], \
                 [6.0, 8.0, 1.0, 0.6, 0.9999, 0.0, 1.0, 1.0, 0.6] \
                 ]
        """
        ...
        #self.k = 20.0#13
        #self.lambda_sq = 1e11# 1e4
        #self.lambda_sp = 1e5 # 1e4
        ...
        self.VelocityMagnitude = 3.0 # magnitude of desired velocity in m/s
        ...
        self.pub = rospy.Publisher('/'+self.uav+'/NavigationFunctionTrajectory', velocities_from_navigation_function, queue_size = 1, tcp_nodelay = True)
        try:
            #tp = rospy.Subscriber('/target_positions_for_quads', target_positions, self.callback1, queue_size = 100, tcp_nodelay = True)
            odom = rospy.Subscriber('/'+self.uav+'/odometry_sensor1/odometry', Odometry, self.callback2, queue_size = 1, tcp_nodelay = True)
        except:
    ...
    def generate_velocity_trajectory(self, phi_, X_, odom): 
        ...
        smoothing_factor = 1/(1+15*np.exp(-2*(t-1)))*scipy.special.erf(1.5*(dist_to_goal-self.proximity))     
        ...
        if nrm(X_-self.goal) > self.proximity and self.phase_one == False:#0#direction_vector[2]#1#direction_vector[0
            ...
            if nrm(v1) == 0: 
                ...
                rospy.loginfo("the target that gave invalid velocity is: %f, %f, %f", self.goal[0], self.goal[1], self.goal[2])
            else: 
            ...
            if self.counter == 1:
                ...
                if self.counter > 200: # number of points used in making spline 
                    ...
                    t = list(np.linspace(min(self.time_points), max(self.time_points), 200))
            ...
            msg.pdes.x = position[0]; msg.pdes.y = position[1]; msg.pdes.z = position[2]
    ...
    def callback2(self, odom): 
        ...
        if self.planning_counter  == 0: 
        ...
        elif self.planning_counter > 0 and self.planning_counter <10000: 
            ...
            #beta = [[(1.0/(1+np.exp(-1.0*j)))-0.5 for j in beta[i]] for i in range(len(beta))]    

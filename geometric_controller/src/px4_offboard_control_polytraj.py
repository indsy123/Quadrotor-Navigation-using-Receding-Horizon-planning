...
class px4_offboard_control(object):
    def __init__(self, name, number, freq, mass, kx, kv, norm_thrust, rate_gain):
        """initializes the variables"""
        self.logpath = '/home/pelican/data_collection/'
        open(self.logpath + "trajectory_subscribed.txt", "w").close()
        open(self.logpath +"trajectory_followed.txt", "w").close()
	open(self.logpath + 'commands.txt', "w").close()
...
if __name__ == '__main__':
    ...
    r = rospy.Rate(100)
    c = px4_offboard_control(name, number, freq, mass, kx, kv, norm_thrust, rate_gain)
